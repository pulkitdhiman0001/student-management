...
def login():
    if request.method == 'POST':
        ...
        exists = db.session.query(db.exists().where(
            Users.username == request.form["username"] and Users.password == request.form["password"])).scalar()
...
def register():
    if request.method == 'POST':
        ...
        confirm_hash_pass = generate_password_hash(confirm_userpass)
        add_user = Users(username=request.form["username"], password=hash_pass,
                         confirm_password=confirm_hash_pass)
